...
class Migrator {
    ...
    getMigrationFiles = async (lastMigrationName, countMigrate) => {
        ...
        if (lastMigrationName) {
            ...
            if (migrationIndexInFilesArray === -1 ) {
                ConsoleLogger.error('Error: The migration files do not match the records in the database. File migration /' + migrationsFolder + migrationName + ' does not Exists')
            } else {
    ...
    upSpecificMigration = async (action, file) => {
        ...
        try {
            if (await migration.up()) {
                ...
                ConsoleLogger.success(' completed')
            }
        } catch (err) {
            console.log()
            ConsoleLogger.error(err)
    ...
    migrationUp = async (action, countMigrate) => {
        ...
        if (!migrationFiles.length) {
            ConsoleLogger.warning('Nothing to up')
            return
        }
    ...
    downSpecificMigration = async (action, files, migrationName) => {
        ...
        ConsoleLogger.message('Migration ' + migrationsFolder + migrationName + ' ' + action + colors.green(' completed'))
    }
    ...
    checkExistsMigrationFiles = async (migrationsFolder) => {
        if (!fs.existsSync(migrationsFolder)) {
            ConsoleLogger.error(`Error: Migrations in ${migrationsFolder} does not Exists`)
            process.exit()
        }
